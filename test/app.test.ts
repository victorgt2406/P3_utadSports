...
describe('stores and review creation', () => {
    ...
    it('should register a store', async () => {
        const response = await request(app)
            .post('/api/store')
            .send({ "name": "store", "cif": "12345", "location" : "madrid", "email": "store@email.com", "phone": "123223432", "activity": "sell" })
            .auth(token, { type: 'bearer' })
            .set('Accept', 'application/json')
            .expect(200)
    ...
    it('should update the webpage', async () => {
        const response = await request(app)
            ...
            .send({webpage:[{"type":"text", "col": 2, "content": "hola"},{"type":"text", "col": 2, "content": "que"},{"type":"text", "col": 3, "content": "pasa"}]})
            .auth(token, { type: 'bearer' })
            .set('Accept', 'application/json')
            .expect(200)
        expect(response.body).toEqual(true)
    ...
    it('should register a user', async () => {
        const response = await request(app)
            .post('/api/user/register')
            .send({ "name": "userreview", "birthdate": "2002-06-24", "email": "userreview@test.com", "password": "HolaMundo.01" })
            .set('Accept', 'application/json')
            .expect(200)
    ...
    it('should get all the reviews and check the update', async () => {
        const response = await request(app)
            ...
            .set('Accept', 'application/json')
            .expect(200)
            expect(response.body.filter((value:any)=>value._id===idReview)[0].msg).toEqual("mensaje de actualizado")
    })
